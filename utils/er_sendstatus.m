function er_sendstatus(to,verbose)
%ER_SENDSTATUS(TO,VERBOSE) sends an exit status report to the mail addresses 
% (specified in TO). In case of failure the function that caused the error
% will be attached. 
%
%HOW DOES IT WORK?
% ER_SENDSTATUS must be placed as the FIRST line of code of a matlab
% script/function.
%
...
% within a try/catch statment

global ER_SENDSTATUS_STATUS

if isempty(ER_SENDSTATUS_STATUS) %first occurance of the current function.
    ...
    if strcmpi(callingFunction.name,'er_sendstatus')
        error('Function er_sendstatus must be called from a script or function.');
    end
    ...
    if exit_status   %ERROR FOUND
        ...
        %remove the last two stacks (useless)
        indx = strfind(error_message,['Error in ',ST(1).name]);
        error_message(indx:end) = [];
    else   %SUCCESS
        subject = ['<',callingFunction.name,'> SUCCESS'];
        msg_str = sprintf('has just successfully completed!\n');
    end
    ...
    if exit_status   %ERROR FOUND
        switch verbose
            ...
            case {2} % the entire stack
                n_found = 1;
                for l = 1:(length(ME.stack)-2) %remove the last two stacks
    ...
    if exit_status  %ERROR FOUND
        ...
        % we have to remove the last two stacks that are useless.
        ...
        errorStruct.stack = ME.stack(1:end-2,1);
        error(errorStruct);
    else %SUCCESS
...
clearvars -global ER_SENDSTATUS_STATUS
return
end
