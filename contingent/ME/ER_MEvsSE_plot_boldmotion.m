function ER_MEvsSE_plot_boldmotion(data,sample_names,paired,sample)
...
for r = 1:5
    subplot(sp_r,sp_c, [((r-1)*sp_c+ 1)]);
    plot([0 (length(sample_names)+1)],[0 0],'k'); hold on
    violin(data.bold_motion.fd_zs{r},'edgecolor','none','xlabel',[],'medc',[],'facealpha',0.7);
    set(gca,'XTick',[]);
...
for r = 1:5
isto_plot(data.bold_motion.fd_zs_mp{r},sample,[sp_r sp_c ((r-1)*sp_c+ 2)],'Positive z-score',sample_names,'z-score',paired,fntsz,fntw);  
 isto_plot(data.bold_motion.fd_zs_mn{r},sample,[sp_r sp_c ((r-1)*sp_c+ 3)],'Negative z-score',sample_names,'z-score',paired,fntsz,fntw);     
end
...
for r = 1:5
    subplot(sp_r,sp_c, [((r-1)*sp_c+ 1)]);
    plot([0 (length(sample_names)+1)],[0 0],'k'); hold on
    violin(data.bold_motion.dvars_zs{r},'edgecolor','none','xlabel',[],'medc',[],'facealpha',0.7);
    set(gca,'XTick',[]);
...
for r = 1:5
isto_plot(data.bold_motion.dvars_zs_mp{r},sample,[sp_r sp_c ((r-1)*sp_c+ 2)],'Positive z-score',sample_names,'z-score',paired,fntsz,fntw);  
 isto_plot(data.bold_motion.dvars_zs_mn{r},sample,[sp_r sp_c ((r-1)*sp_c+ 3)],'Negative z-score',sample_names,'z-score',paired,fntsz,fntw);     
end
return
end
