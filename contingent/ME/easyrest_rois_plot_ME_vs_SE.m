function easyrest_rois_plot_ME_vs_SE(ER_)
% hardcoded for plotting results for all hands HCP meeting
...
ER.stats.paired = 1;                 % 1/0; 1: perform a paired t-test (when it is possible). ATTENTION: Affect both BAR plot and ROI-to-ROI stats
...
ER.bar_plot_options.plot_type = 3;           % [1,2,3]; 1: bar plot; 2: box plot; 3: line plot
...
ER.sample(1).corr(1).cova = [];     % only for PARTIAL CORRELATION. If you want to remove the effect of confounds (e.g., age, sex, score) put in the matrix corr(m).cova. cova is a mxn matrix= m runs on subjects while n on covariates. 
...
ER.sample(1).RtoR_cova = [];        % covariates only for ROI to ROI plots
...
for s = 1:sample_number
    ...
    pariredbility_check = [pariredbility_check,ER.sample(s).tot_subj];
end
...
if length(unique(pariredbility_check)) ~=1 % i sample hanno un numero diverso di soggetti
    if ER.stats.paired
...
if ER.bar_plot
    ...
    for l = ER.indx_roi
        for i = 1:measure_number
            ...
            isto(isto_cell, ER.sample, sample_number, sample_names);
        ...
        for c = 1:length(ER.sample(s).corr)
            ...
            if ER.correlation_partial
                if isfield(ER.sample(s).corr(c),'cova') && ~isempty(ER.sample(s).corr(c).cova)
                   ...
                   ER.sample(s).corr(c).cova_N = max(s_cova);
                end
            end
        end
        ...
        for s =1:sample_number
            ...
            for c = 1:ER.sample(s).corr_number
                ...
                if isempty(ER.correlation_only_below_thr)       %caso senza threshold
                    figure;
                    set(gcf,'name',[ER.correlation_type,' correlation Plot of ',sample_names{s},' with: ',ER.sample(s).corr(c).name])
                    for l = ER.indx_roi
                        for i = 1:measure_number
                            count = count +1;
                            subplot(s_p_row, s_p_col, count)
...
%                     print('-dpng','-r400',[sample_names{s},'_',ER.sample(s).corr(c).name,'.png'])
                    ...
                    h = waitbar(0,'Looking for significant correlations...','Name',['Correlation of ',sample_names{s},' with ',ER.sample(s).corr(c).name]);
                    count = 0;
                    for l = ER.indx_roi
                        for i = 1:measure_number  
                            count = count +1;
...
if exist('DA','var') && ~isempty(DA) && ER.DA_plot
    ...
    data = da_create_sample(DA_extracted,selectors,good_subjs);
    data.d_vect = DA_extracted.roitoroi.d_vect{1,1,1,1};
    %---------------
    %plot:
    wd = pwd;
    cd (DA.folder_name)
    
    %Plot tsnr and others
    ER_MEvsSE_plot_tsnr(data,sample_names,flag_paired,ER.sample)
   %Plot specificity
   %ER_MEvsSE_plot_specificity(data,sample_names,flag_paired,ER.sample)
    % plot QC movements
  % ER_MEvsSE_plot_qc_movements(data,sample_names,flag_paired,ER.sample)
    % plot BOLD:movements
  % ER_MEvsSE_plot_boldmotion(data,sample_names,flag_paired,ER.sample)
    % plot QC-RSFC
   % ER_MEvsSE_plot_qc_rsfc(data,sample_names)
    da_2ndl_plot(data,'USER_DEFINED',sample_names,'USER_DEFINED',flag_paired,1,data.names_reg,'')
    cd (wd);
end
...
if ER.RtoR.plot
    ...
    if ER.RtoR.ostt.do
        ...
        save('_ER_plotted_data_RtoR_OSTT','ostt');
    end
    ...
    if ER.RtoR.tstt.do
        ...
        if flag_paired 
            ...
            if size1~=size2
                ...
                warning('Sorry, something goes wrong! Paired t-test cannot be performed, switching to unpaired modality.');
                warning backtrace on
                flag_paired = 0;
            else
                % OLD WAY (as FLS does). PROBLEM HERE IS COVARIATE. IF YOU
                % ADD A COVARIATE X BECOMES SINGULAR.  
...
%                 C = 1;
                ...
                X = [ones(size(z_corr,1),1)];
                C = 1;
                % NOTE: the two modality are matematically equivalent, no
        ...
        if (ER.sample(selector(1)).RtoR.cova_do + ER.sample(selector(2)).RtoR.cova_do) == 2
            ...
            if flag_paired 
                cova = ER.sample(selector(1)).RtoR.cova;
            end
        ...
        save('_ER_plotted_data_RtoR_TSTT','tstt');
    end
...
function isto(dati,sample,sample_number,sample_names)
...
data.colors = DA.colors;
data.names_reg = DA.name_contract;
data.names_spec = DA.specifcity_names;

data.fd = da_create_sample_aux(selectors,DA.fd,good_subjs);
data.fd_TRnorm = da_create_sample_aux(selectors,DA.fd_TRnorm,good_subjs);
data.fd_rms = da_create_sample_aux(selectors,DA.fd_rms,good_subjs);
data.fd_TRnorm_rms = da_create_sample_aux(selectors,DA.fd_TRnorm_rms,good_subjs);
...
data.mean_dvars = da_create_sample_aux(selectors,DA.mean_dvars,good_subjs);
...
for r = 1:size(DA.vTv,4)
    ...
    data.tSNR_mean{r} = da_create_sample_aux(selectors,DA.tSNR_mean,good_subjs,r);
    ...
    data.bold_motion.fd_zs_mp{r} = da_create_sample_aux(selectors,DA.bold_motion.fd_zs_mp,good_subjs,r);
    %DA_extracted.bold_motion.fd_zf_mn{g,j,k,count} = DA.reg(r).bold_motion.fd.zf_mean_n;
    data.bold_motion.fd_zs_mn{r} = da_create_sample_aux(selectors,DA.bold_motion.fd_zs_mn,good_subjs,r);
    ...
    data.bold_motion.dvars_zs_mp{r} = da_create_sample_aux(selectors,DA.bold_motion.dvars_zs_mp,good_subjs,r);
    %DA_extracted.bold_motion.dvars_zf_mn{g,j,k,count} = DA.reg(r).bold_motion.dvars.zf_mean_n;
    data.bold_motion.dvars_zs_mn{r}= da_create_sample_aux(selectors,DA.bold_motion.dvars_zs_mn,good_subjs,r);
    
    
    data.bold_motion.fd_zs{r} =da_create_sample_aux(selectors,DA.bold_motion.fd_zs,good_subjs,r);
    data.bold_motion.dvars_zs{r} = da_create_sample_aux(selectors,DA.bold_motion.dvars_zs,good_subjs,r);
    %--------------------

    %----Roi-to-ROi
    data.roitoroi.zs.vect{r} = da_create_sample_aux(selectors,DA.roitoroi.zs.vect,good_subjs,r);
...
for s = 1:sample_number
    ...
    for ss = 1: size(selectors{s},1)
        ...
        for k = 1:subject_number(group_sel)
            if nargin == 4
                if length(y{group_sel,sess_sel,k,reg_n}) > 1  && length(y{group_sel,sess_sel,k,reg_n})~=199 %case histogram!  %variable size vector case
                    if ~isempty(good_subjs{s}) && good_subjs{s}(k) == 0
