function easyrest_rois_plot_MOD(ER_)
...
ER.bar_plot_options.paired = 1;              % 1/0; 1: perform a paired t-test (when it is possible)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clearvars -global sample_number sample_names temp_rois filter_band
...
flag_paired = ER.bar_plot_options.paired ;
...
%                             end
                    ...
                    % to remove
                    %set(gcf,'units','centimeters','position',[0 0 10 3]);
                    %set(gcf,'position',[0 0 1 0.5]);
                    set(gcf,'paperunits','centimeters','paperposition',[0 0 20 4])
                    set(findall(gcf, '-property', 'FontSize'), 'FontSize', 5, 'fontWeight', 'normal')
                    set(findobj(gca,'ylabel','text'),'FontSize',7)
                    set(findall(gca, '-property', 'Linewidth'), 'Linewidth', 0.8)
...
if ER.RtoR.plot
    ...
    if ER.RtoR.tstt.do
        ...
        [tstt(1).T,tstt(1).P] = RtoR_glm(z_corr,X,[1 -1]);
    ...
    if ER.RtoR.corr.do
        for s =1:sample_number
            ...
            for c = 1:ER.sample(s).corr_number
                figure_title = ['ATLAS: ',atlas_name,' Correlation Plot of ',sample_names{s},' with: ',ER.sample(s).corr(c).name];
                data_x = ER.sample(s).corr(c).vect;
...
global corr_type
hs = scatter(x,y,7,'filled','o','MarkerFaceColor',[0.4 0.4 0.4],'MarkerEdgeColor',[0.4 0.4 0.4]);  
hold on
...
if sum(a)>3	    
    ...
    if p <= 0.05
        ...
        c2= plot(cc2);
        set(c2,'Color',[180/255,0,0],'Linewidth',1);
