function easyrest_rois_extract_beta_GMWMf(ER_)
...
for g = 1:opt.group_number 
    for v = 1:opt.subject_number(g)
        ...
        gmmask = spm_read_vols(spm_vol(c_b(opt.DATA.WC1{g}(v,:))));
        wmmask = spm_read_vols(spm_vol(c_b(opt.DATA.WC3{g}(v,:))));
        gmmask(gmmask <= thr) = 0;
        gmmask(gmmask > thr) = 1;
        wmmask(wmmask <= thr) = 0;
        wmmask(wmmask > thr) = 1;
            ...
            for m = 1: length(measure)
                ...
                for r = 1: roi_classes
                    for l = 1: length(roi_class(r).roi)
                        ...
                        tmp = volume(bmask==1 & gmmask == 1);
                        %tmp(tmp==0) = [];
                        data_gm(g,v,s,m,r,l) = er_nanmean(tmp);
                        %data_gm(g,v,s,m,r,l) = sum(gmmask(:));
                        
                        tmp = volume(bmask==1);
                        %tmp(tmp==0) = [];
                        data_wm(g,v,s,m,r,l) = er_nanmean(tmp);
                        %data_wm(g,v,s,m,r,l) = sum(wmmask(:));
